...
train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=hp.TRANSFORMER_BATCH_SIZE, collate_fn=SmartwatchAugmentTransformer(max_input_samples=512, downsample_output_seq=1, num_heads=hp.NUM_HEADS), drop_last=True, shuffle=True)

val_dataset = SmartwatchDataset(val_files, sample_period=0.04)
val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=hp.TRANSFORMER_BATCH_SIZE, collate_fn=SmartwatchAugmentTransformer(max_input_samples=512, downsample_output_seq=1, num_heads=hp.NUM_HEADS), drop_last=True, shuffle=True)

test_dataset = SmartwatchDataset(test_files, sample_period=0.04)
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=hp.TRANSFORMER_BATCH_SIZE, collate_fn=SmartwatchAugmentTransformer(max_input_samples=512, downsample_output_seq=1, num_heads=hp.NUM_HEADS), drop_last=True, shuffle=False)
